...
export async function GET(request, { params }) {
  ...
  if (!await getAdminUser(supabase)) {
    return NextResponse.json({ success: false, message: 'error', data: { details: 'Forbidden: Admin access required.' } }, { status: 403 });
  }
  ...
  if (error || !property) {
    return NextResponse.json({ success: false, message: 'error', data: { details: 'Property not found.' } }, { status: 404 });
  }
...
export async function PUT(request, { params }) {
  ...
  if (formData.has('description')) propertyUpdates.description = formData.get('description');
  if (formData.has('host_name')) propertyUpdates.host_name = formData.get('host_name');
  if (formData.has('host_phone')) propertyUpdates.host_phone = formData.get('host_phone');
  if (formData.has('host_email')) propertyUpdates.host_email = formData.get('host_email');
  if (formData.has('status')) propertyUpdates.status = formData.get('status');

  // Handle numbers
  if (formData.has('price_per_night')) propertyUpdates.price_per_night = parseFloat(formData.get('price_per_night'));
  if (formData.has('max_guests')) propertyUpdates.max_guests = parseInt(formData.get('max_guests'));
  if (formData.has('num_bedrooms')) propertyUpdates.num_bedrooms = parseInt(formData.get('num_bedrooms'));
  if (formData.has('latitude')) propertyUpdates.latitude = parseFloat(formData.get('latitude'));
  if (formData.has('longitude')) propertyUpdates.longitude = parseFloat(formData.get('longitude'));
  if (formData.has('province_id')) propertyUpdates.province_id = parseInt(formData.get('province_id'));
  ...
  if (newImages.length > 0 || imagesToRemove.length > 0) {
    const newImageUrls = await uploadPropertyImages(supabase, newImages, propertyId);
  ...
  if (updateError) {
    return NextResponse.json({ success: false, message: 'error', data: { details: updateError.message } }, { status: 500 });
  }
...
export async function DELETE(request, { params }) {
  ...
  if (deleteError) {
    return NextResponse.json({ success: false, message: 'error', data: { details: deleteError.message } }, { status: 500 });
  }
