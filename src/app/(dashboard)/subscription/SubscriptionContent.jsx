...
export default function SubscriptionContent({ isPro, isCanceling, currentPeriodEnd }) {
  const { t } = useTranslation();

  const freeFeatures = [
    t("subscription.free_feature_1"),
    t("subscription.free_feature_2"),
  ];

  const proFeatures = [
    t("subscription.pro_feature_1"),
    t("subscription.pro_feature_2"),
    t("subscription.pro_feature_3"),
  ];
  ...
  return (
    <section className="w-full max-w-5xl mx-auto py-12 md:py-4 px-6 md:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        >
          <CardHeader>
            <p className="text-sm font-medium text-gray-500">
              {t("subscription.for_casual")}
            </p>
            <CardTitle className="text-4xl">{t("subscription.free")}</CardTitle>
            <CardDescription>
        ...
        >
          <CardHeader>
            <p className="text-sm font-medium">{t("subscription.for_frequent")}</p>
            <CardTitle className="text-4xl">{t("subscription.pro")}</CardTitle>
            <CardDescription>
              {t("subscription.pro_desc")}
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-6 flex-1 p-6">
                  ...
                  <div className="w-full">
                    {!isCanceling ? (
                      ...
                      <div className="w-full h-10 rounded-md bg-yellow-50 border border-yellow-200 text-yellow-700 flex items-center justify-center gap-2 text-sm font-medium">
                        ...
                        {t("subscription.cancels_on", { date: currentPeriodEnd })}
                      </div>
                    )}
                  </div>
                </>
